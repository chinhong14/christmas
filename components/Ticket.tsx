...
// Tree Icon Component
const TreeIcon = () => (
  <svg viewBox="0 0 24 24" fill="none" className="w-10 h-10 drop-shadow-[0_0_8px_rgba(234,179,8,0.8)]" xmlns="http://www.w3.org/2000/svg">
...
const Ticket: React.FC<TicketProps> = ({ name }) => {
  ...
  return (
    <div className="flex flex-col items-center gap-8 animate-fade-in-up w-full max-w-6xl px-4 md:px-0">
      
      {/* TICKET WRAPPER */}
      <div 
        className={`relative flex w-full filter drop-shadow-[0_0_40px_rgba(234,179,8,0.4)] transition-transform duration-500 ${isMobile ? 'flex-col items-center w-[340px]' : 'flex-row items-stretch'}`}
      >
        ...
        <div 
            ...
            className={`
                ...
                ${isMobile 
                    ? 'w-full rounded-t-3xl border-b-0 min-h-[480px]' 
                    : 'flex-grow rounded-l-3xl border-r-0 pr-8 min-h-[380px]'}
                p-8 md:p-12 flex flex-col justify-between text-left
            `}
        >
            ...
            <div className="relative z-10 flex flex-col h-full text-left">
                ...
                {/* Header */}
                <div className="mb-10">
                    <h2 className="font-script text-4xl md:text-5xl text-gold-400 mb-2 transform -rotate-2 origin-bottom-left drop-shadow-md">
                        Kundasang Gang
                    </h2>
                    <h1 className="font-serif text-4xl md:text-6xl text-transparent bg-clip-text bg-gradient-to-br from-gold-100 via-gold-300 to-gold-600 drop-shadow-sm uppercase tracking-wide">
                        Christmas Party
                    </h1>
                </div>
                ...
                <div className="flex flex-col items-start mb-auto w-full max-w-md">
                     ...
                     <div className="font-serif text-5xl md:text-6xl text-white tracking-wide mb-2">
                        {name || "Guest"}
                     </div>
        ...
        >
            ...
            {/* Cutout Circles */}
            <div className={`absolute bg-slate-900 border border-gold-500/50 rounded-full z-10 w-5 h-5 ${isMobile ? '-left-2.5 top-1/2 -translate-y-1/2' : '-top-2.5 left-1/2 -translate-x-1/2'}`}></div>
            <div className={`absolute bg-slate-900 border border-gold-500/50 rounded-full z-10 w-5 h-5 ${isMobile ? '-right-2.5 top-1/2 -translate-y-1/2' : '-bottom-2.5 left-1/2 -translate-x-1/2'}`}></div>
        ...
        <div 
            className={`
                ...
                ${isMobile 
                    ? `w-full h-44 rounded-b-3xl border-t-0 ${isTorn ? 'animate-tear-off-vertical' : ''}` 
      ...
      <div className={`transition-all duration-700 ease-in-out z-50 ${isTorn ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10 pointer-events-none'}`}>
        <button
          ...
          className="
            ...
            text-slate-900 font-serif tracking-widest text-sm font-bold
